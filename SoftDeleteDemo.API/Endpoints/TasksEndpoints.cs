...
namespace SoftDeleteDemo.API.Endpoints
{
	public class TasksEndpoints : IEndpoints
	{
		...
		private async Task<IResult> GetTasksByUser(int userId, TaskService taskService, bool includeDeleted = false)
		{
		...
		private async Task<IResult> GetAllTasks(TaskService taskService, bool includeDeleted = false)
		{
			var tasks = includeDeleted 
				? await taskService.GetAllTasksIncludeDeletedAsync() 
